...
main() {
  ...
  if [[ -d "${data_dir}" ]]; then
    ...
    if [[ $platform == "mingw"* ]]; then
      ...
      package_name=lite-xl$version-windows-$platform
    fi
  ...
  # TODO: use --skip-subprojects to avoid include and libs copy
  # when 0.58.0 will be available on supported distributions.
  # Install Meson with PIP to get the latest version is not always possible.
  rm -rf "${dest_dir}/include" "${dest_dir}/lib" "${dest_dir}/msys64"
